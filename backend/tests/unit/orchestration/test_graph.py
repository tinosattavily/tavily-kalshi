...
async def test_run_analysis_graph_full_execution():
    ...
    with (
        patch("app.orchestration.graph.run_market_agent") as mock_market,
        patch("app.orchestration.graph.run_event_agent") as mock_event,
        patch("app.orchestration.graph.run_search_planner") as mock_tavily,
        patch("app.orchestration.graph.run_article_fetcher") as mock_news,
        patch("app.orchestration.graph.run_summarizer") as mock_summary,
        patch("app.orchestration.graph.run_probability_agent") as mock_prob,
        patch("app.orchestration.graph.run_strategy_agent") as mock_strategy,
        patch("app.orchestration.graph.run_report_agent") as mock_report,
    ):
...
async def test_run_analysis_graph_early_termination():
    ...
    with patch("app.orchestration.graph.run_market_agent") as mock_market:
...
async def test_run_analysis_graph_error_propagation():
    ...
    with patch("app.orchestration.graph.run_market_agent") as mock_market:
        mock_market.side_effect = RuntimeError("Market agent error")
